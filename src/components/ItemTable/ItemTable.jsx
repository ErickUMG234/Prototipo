...
const ItemTable = ({ item }) => {
        ...
        axiosInstance.delete(`/Materiales/${id_material}`)
            .then(r => {
                if (r.status === 200) {
                    ...
                    // Filtrar los items actualizados sin el item eliminado
                    const updatedItems = items.filter(item => item.id_material !== id_material);
                    console.log(updatedItems);
                    
                    // Actualizar el estado global con los items filtrados
    ...
    const handleUpdate = async (id_material, values) => {
        ...
        try {
            ...
            setModalShow(false);  // Cierra el modal después de actualizar
        } catch (error) {
        ...
        <>
            <tr>
                ...
                <td style={{ display: 'flex', justifyContent: 'space-evenly' }}>
                    ...
                    <i className="bi bi-pencil-square" style={{ cursor: 'pointer' }} onClick={() => { console.log(item); setModalShow(true); }}> editar</i>
                </td>
            </tr>
            <Modal
                show={modalShow}
